...
class omega( chem_evol ):
    ...
    def __init__(self, galaxy='none', in_out_control=False, SF_law=False, \
                 ...
                 delayed_extra_yields_log_int=False, prim_comp_table='',\
                 dt_in_SSPs=np.array([]), \
...
#### trueman edits
    ...
    def mass_frac_plot(self,fig=0,species=['all'],sources=['agb','massive','1a'],\
                      cycle=-1, solar_ref='Lodders_et_al_2009',yscale='linear',
                      col=['blue','orange','grey','navy','green'],xaxis_lab='name',
                      marker='x',marker_size=20, facecolor=None, show_legend=True,
                      show_title=True, delayed_source_name='delayed_extra_source'):
        ...
        for ele,mass in ele_dict.items():
            for el,abu in sol_dict.items():
                if ele == el:
                    total_mass += mass*(10**(abu-12))

        for ele,mass in ele_dict.items():
            for el,abu in sol_dict.items():
                if ele == el:
                    ele_mass_frac_dict.update([(ele,
                        (10**(abu-12))*mass/total_mass)])

        total_abufrac_x_isomass = {}
        for ele, massfrac in ele_mass_frac_dict.items():
            iso_x_abufrac = 0
            for iso, abufrac in iso_frac_dict.items():
                if iso.split('-')[0]==ele:
                    iso_x_abufrac += float(iso.split('-')[1])*abufrac
            total_abufrac_x_isomass.update([(ele,iso_x_abufrac)])

        iso_mass_frac_dict = {}
        for ele, total_abufrac in total_abufrac_x_isomass.items():
            for iso, abufrac in iso_frac_dict.items():
                if iso.split('-')[0] == ele:
                    iso_mass_frac_dict.update([(iso,
                    float(iso.split('-')[1])*abufrac/total_abufrac)])

        weighted_iso_frac = {}
        for ele, ele_mass_frac in ele_mass_frac_dict.items():
            for iso, iso_mass_frac in iso_mass_frac_dict.items():
                if iso.split('-')[0]==ele:
                    weighted_iso_frac.update([(iso,
                        iso_mass_frac*ele_mass_frac)])

        species_mass_frac_sol_dict = weighted_iso_frac
        species_mass_frac_sol_dict.update(ele_mass_frac_dict)
        ...
        # This dictionary contains the mass fraction contribution toward the total
        # by massive stars for each species
        ...
        iso_mass_nsm = dict(zip(self.history.isotopes, self.ymgal_nsm[cycle]/sum(self.ymgal[cycle])))

        ele_mass_nsm = np.zeros(len(elements))
        i=0

        for el in elements:
            for iso,mass in iso_mass_nsm.items():
                mass = float(mass)
                if el == iso.split('-',1)[0]:
                    ele_mass_nsm[i] += mass
            i+=1
        ele_mass_nsm_dict = dict(zip(elements, ele_mass_nsm))
        species_mass_nsm = iso_mass_nsm
        species_mass_nsm.update(ele_mass_nsm_dict)

        species_frac_nsm = {k: species_mass_nsm[k] / species_mass_gal[k]
        for k in species_mass_nsm if k in species_mass_gal}
 
        species_mass_delayed = {} 
        species_frac_delayed = {}
        for i_source in range(len(self.ymgal_delayed_extra)):
            iso_mass_delayed = dict(zip(self.history.isotopes,
                self.ymgal_delayed_extra[i_source][cycle]/sum(self.ymgal[cycle])))
            ele_mass_delayed = np.zeros(len(elements))
            i=0

            for el in elements:
                for iso, mass in iso_mass_delayed.items():
                    mass = float(mass)
                    if el == iso.split('-',1)[0]:
                        ele_mass_delayed[i] += mass
                i+=1

            ele_mass_delayed_dict = dict(zip(elements, ele_mass_delayed))
            species_mass_delayed[i_source] = iso_mass_delayed
            species_mass_delayed[i_source].update(ele_mass_delayed_dict)

        #iso_mass_delayed = dict(zip(self.history.isotopes, 
         #   self.ymgal_delayed_extra[0][cycle]/sum(self.ymgal[cycle])))
   ...
   #             mass = float(mass)
    #            if el == iso.split('-',1)[0]:
     #               ele_mass_delayed[i] += mass
      #      i+=1     
       # ele_mass_delayed_dict = dict(zip(elements, ele_mass_delayed))
        #species_mass_delayed = iso_mass_delayed
       # species_mass_delayed.update(ele_mass_delayed_dict)

        # This dictionary contains the mass fraction contribution toward the total
        # by extra sources
        for i_source in species_mass_delayed.keys():
            species_frac_delayed[i_source] = {k: species_mass_delayed[i_source][k] / species_mass_gal[k]
                    for k in species_mass_delayed[i_source] if k in species_mass_delayed[i_source]}
            
        map_str_dic = {
            "agb":species_mass_agb,
            "1a":species_mass_1a,
            "massive":species_mass_massive,
            "nsm":species_mass_nsm
        }

        for i_source in species_frac_delayed.keys():
            map_str_dic['extra_delayed_'+str(i_source)]=species_mass_delayed[i_source]

        source_proper_name = {
            "agb":'AGB',
            "1a":'SN1a',
            "massive":'Massive stars',
            "nsm":"Neutron star mergers"
        }

        for i_source in species_frac_delayed.keys():
            source_proper_name['extra_delayed_'+str(i_source)]=delayed_source_name[int(i_source)]
        ...
        for source in sources:
            ...
            for spe in species:
                if spe in species_mass_agb:
                    ...
                    if xaxis_lab == 'numeric':
                        iso = float(spe.split('-')[1])
                        plt.scatter(iso,val, color=colors[j],marker=marker,
                                s=marker_size,facecolor=facecolor),
                    elif xaxis_lab == 'species':
        ...
        if xaxis_lab == 'numeric' or xaxis_lab == 'species':
            plt.yscale(yscale)
    ...
    def plot_spectro(self,fig=3,xaxis='age',yaxis='[Fe/H]',source='all',label='',
            shape='-',marker='o',color='k',markevery=100,show_data=False,show_sculptor=False,
            show_legend=True,return_x_y=False,sub_plot=False,linewidth=3,sub=1,plot_data=False,
            fsize=[10,4.5],fontsize=14,rspace=0.6,bspace=0.15,labelsize=15,legend_fontsize=14,
            only_one_iso=False,solar_ab='',sfr_thresh=0.0,m_formed_thresh=1.0,solar_norm=''):
        '''
    ...
    def plot_sn_distr(self,fig=5,rate=True,rate_only='',xaxis='time',fraction=False,label1='SNIa',label2='SN2',shape1=':',shape2='--',marker1='o',marker2='s',color1='k',color2='b',markevery=20,fsize=[10,4.5],fontsize=14,rspace=0.6,bspace=0.15,labelsize=15,legend_fontsize=14):
        ...
        if rate and not fraction:
            ...
            if fraction:
                ...
                ratio=[]
                age1=[]
                for k in range(len(self.wd_sn1a_range)):
                    if self.wd_sn1a_range[k]>0:
                        ratio.append(self.history.sn1a_numbers[1:][k]/self.wd_sn1a_range[k])
                        age1.append(age[k])
                plt.plot(age1,ratio)
                plt.yscale('log')
                #plt.xscale('log')
    ...
    def plot_mdf(self, fig=19, return_x_y=False, axis_mdf = '[Fe/H]',\
        ...
        for i_sim in range(0, len(x_all)-1):
          ...
          if np.isfinite(x_all[i_sim]) and np.isfinite(x_all[i_sim+1]):
            ...
            if (x_all[i_sim+1] - x_all[i_sim]) == 0:
                f_mlocked = 1
            else:
